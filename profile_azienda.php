...
$SQLideeU="SELECT TITOLO,P.ID_UTENTE,I.DESCRIZIONE FROM IDEE I JOIN PERSONE P ON CREATORE=P.ID_UTENTE JOIN AZIENDE A ON AZIENDA=A.ID_UTENTE WHERE A.ID_UTENTE='".$id."'";
$result_ideeU=$mysqli->query($SQLideeU);

//per conoscere il nuemro di idee degli UTENTI
$SQLnum_ideeU="SELECT COUNT(ID) AS IDEEU FROM IDEE I JOIN PERSONE P ON CREATORE=P.ID_UTENTE JOIN AZIENDE A ON AZIENDA=A.ID_UTENTE WHERE A.ID_UTENTE='".$id."'";
$result1_ideeU=$mysqli->query($SQLnum_ideeU);
$num_ideeU=mysqli_fetch_assoc($result1_ideeU);

// per le idee sviluppate dall azienda
$SQLideeA="SELECT TITOLO,DESCRIZIONE FROM IDEE WHERE CREATORE='".$id."'";
$result_idee=$mysqli->query($SQLideeA);

// per conoscere il nuemro di idee dell Azienda
$SQLnum_idee="SELECT COUNT(ID) AS IDEEA FROM IDEE WHERE CREATORE='".$id."'";
$result1_idee=$mysqli->query($SQLnum_idee);
$num_ideeA=mysqli_fetch_assoc($result1_idee);
...
<div class="col-md-3">
		...
		<img class="profile-user-img img-responsive center-block" src='
			...
			<b>Idee pubblicate Azienda</b> <a class="pull-right"><?php echo $num_ideeA["IDEEA"];?></a>
			            </li>
			<li class="list-group-item">
			<b>Idee pubblicate Utenti</b> <a class="pull-right"><?php echo $num_ideeU["IDEEU"];?></a>
			            </li>
			<li class="list-group-item">
            	<a data-toggle="modal" data-target="#Reputazione"><b>Reputazione</b><span class="pull-right"> <?php echo $dati["REPUTAZIONE"];?> </span></a>
			            </li>
			            <?php require_once(TEMPLATES_PATH . "/reputation-info.php");?>
			        </ul>
			 </h6>			
  					     </p>
  					</a>
		...
		<!-- AddToAny END -->
			...
			<?php
                echo "<strong><i class='fa fa-envelope margin-r-5'></i> Email </strong>
                  <p>".$dati['EMAIL']."
                  </p><hr>";	?>
			 <?php
			 ...
			 <?php
                echo "<strong><i class='fa fa-credit-card margin-r-5'></i> Partita Iva</strong>
                  <p>".$dati['PARTITA_IVA']."
                  </p><hr>";	?>
			 <?php 
			...
			<ul class="nav nav-tabs">
				<li role="presentation" class="dropdown">
					...
					<ul class="dropdown-menu">
						<li class="active"><a href="#IdeeAzienda" data-toggle='tab'>dell'Azienda</a></li>
		...
		<div class="tab-content">			  	
  		...
  		<!-- panello su Idee degli UTENTI -->
		        <div class="tab-pane " id="IdeeUtenti">
			        ...
			        if( $num_ideeU["IDEEU"]==0)
			        	echo "Nessun idea pubblicata dagli utenti dell'azienda.";
			        ?>
			        </div>
	
		<!-- pannello sulle ide dell AZIENDA -->
				<div class="tab-pane fade in active" id="IdeeAzienda">
			        ...
			        if($num_ideeA["IDEEA"]==0)
			        	echo"Nessun idea pubblicata dall' azienda."
			        ?>
			    </div>
